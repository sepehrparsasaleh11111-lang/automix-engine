...
└─────────────────────────────────────────────────────────────┘
            │
┌───────────▼─────────────────────────────────────────────────┐
│        openmix-app — Storage (rusqlite / SQLite)            │
│   projects · tracks · analysis cache · mix presets ·        │
│   preferences · renders                                     │
└─────────────────────────────────────────────────────────────┘
...
- `openmix-core` — engine crate. Zero Tauri dependencies, zero persistence
  (no rusqlite/SQLite). Every module is unit-testable with plain `cargo test`.
  CLI harness for headless analysis/render testing.
- `openmix-app` — Tauri 2 application crate. Thin: maps IPC commands to core APIs,
  owns the app event loop, plugin wiring, **and all persistence** (SQLite storage
  module; core results are serialized by the app).
...
Analysis results are cached by the app layer in SQLite (per-track
`track_analysis` cache table, keyed by file hash); re-analysis only when
the file hash or settings change. `openmix-core` itself never persists —
it returns serializable results.

**Native dependency isolation (aubio, KeyFinder):** both are C/C++ libraries
compiled at build time. They are gated behind the `native-analysis` cargo
feature of `openmix-core` (enabled by default) and only reachable through the
detector traits above. Building without the feature compiles pure-Rust
fallback stubs (autocorrelation tempo, K-S chroma key, spectral-flux onsets)
behind the same traits. This keeps Windows builds manageable: if a native
dependency misbehaves, it can be disabled per-platform in CI or by the user.
...
- Correction — ownership is split by phase:
  - **Automatic (Phase 2, backend):** re-fit with tighter windows when
    confidence is low; detect tempo drift by residual analysis. Runs
    headlessly in core; covered by fixture tests in Phase 2.
  - **Manual (Phase 3, interactive UI):** the UI edits first-beat offset,
    BPM, and per-bar markers; every edit lands in SQLite and triggers
    re-fit of dependent data (grid markers, phrase estimates). The
    backend correction logic from Phase 2 powers the edit re-fits.
...
- **Encoder strategy (decided at Phase 0):**
  - WAV (16/24-bit PCM) and FLAC: written via `symphonia`'s encoder support.
  - MP3: **LAME via `lame-rs` bindings** (C library, LGPL). Audio quality is
    the project's #1 priority and LAME is the reference MP3 encoder; a pure-Rust
    MP3 encoder is not used at this quality bar. The encoder lives behind a
    replaceable `Encoder` trait so the implementation can change without
    touching the pipeline.
  - Defaults: MP3 320 kbps CBR (VBR preset available), WAV 16/24-bit, FLAC level 5.
  - LGPL notice: attribution added to the app's about/legal screen and
    README (required by LAME's license).
...
### 2.6 Storage (openmix-app — not part of core)

Persistence is an application-layer concern. `openmix-core` never touches a
database and never reads/writes user files beyond the streams it is given;
the app layer owns all persistence:

- `rusqlite` (bundled SQLite) lives in `openmix-app/src/storage/`.
- Core returns serializable result types (analysis, grids, render jobs);
  the app persists them and serves them back to the UI.
- Tables: `projects`, `tracks`, `track_analysis`, `beat_grids`,
  `mix_presets`, `preferences`, `renders`.
...
- Typed `AppError` in core (decode, analysis, grid, render, io). The app
  layer maps its own storage errors (rusqlite) at the command boundary.
...
|-------|-------------|------|
| 0 | architecture.md, build guide, deps, folder structure, README, CI, repo | docs committed, repo live, CI workflow live |
| 1 | Tauri+React+SQLite skeleton (storage in app crate), import MP3/WAV/FLAC, waveform, project CRUD | app opens & manages projects |
| 2 | analysis pipeline: BPM, beat grid with **automatic backend correction**, key, energy | ≥90% accuracy on fixture suite |
| 3 | AutoMix: beat match, transition selection, crossfade, EQ, effects + **interactive beat-grid editing UI** (manual correction) | beat-matched auto-mix + manual grid edits work in UI |
| 4 | render to MP3/WAV/FLAC (LAME MP3), background progress, 2-hour stability | stable long render, bounded RAM |
